...
async function main() {
  ...
  if (!existingSuperAdmin) {
    const superAdminEmail = process.env.SUPER_ADMIN_EMAIL || 'superadmin@quiz.com';
    ...
    console.log(`ℹ️ SUPER_ADMIN user already exists (${existingSuperAdmin.email}). Skipping initial creation.`);
  }

  // 2. Ensure initial Host user exists for demo quizzes
  let host = await prisma.user.findUnique({ where: { email: 'host@quiz.com' } });
  if (!host) {
  ...
  let admin = await prisma.user.findUnique({ where: { email: 'cognition@gim.ac.in' } });
  if (!admin) {

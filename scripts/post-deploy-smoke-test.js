...
async function runSmokeTests() {
  ...
  await checkStep("GET /api/live (Process Liveness)", async () => {
    ...
    if (data.status !== "up")
      throw new Error(`Expected status 'up', got '${data.status}'`);
  });
  ...
  try {
    await fetchUrl(`${targetUrl}/room/reset-load-test/999999`, {
      method: "POST",
    });
  } catch {
        ...
        socket.emit(
          ...
          (ack) => {
            ...
            if (ack && ack.success) {
              ...
              ack &&
              ack.message &&
              ack.message.includes("not found")
            ) {
  ...
  if (failed > 0) {
    console.error(
      `❌ POST-DEPLOYMENT SMOKE TESTS FAILED! ABORTING DEPLOYMENT.`,
    );
    process.exit(1);
  } else {

...
export const apiRequest = async (path: string, options: RequestInit = {}, retryCount = 0): Promise<any> => {
  ...
  if (response.status === 401 && retryCount === 0 && !path.includes('/auth/login') && !path.includes('/auth/refresh')) {

...
import { Shield, Sparkles, Mail, Lock, User, CheckCircle2, ShieldAlert } from 'lucide-react';
...
export default function AuthPage() {
  ...
  const handleGoogleMock = async () => {
    ...
    try {
      const mockEmail = email || `oauth-${Math.floor(Math.random() * 100000)}@gmail.com`;
  ...
  return (
    <div className="relative min-h-screen gradient-dark flex flex-col justify-center items-center text-slate-100 px-4 py-12 overflow-hidden">
      ...
      <div className="mb-8 text-center relative z-10 cursor-pointer" onClick={() => router.push('/')}>
        ...
        <h2 className="text-3xl font-extrabold tracking-tight">AuraQuiz Portals</h2>
        <p className="text-slate-400 text-xs mt-1">Enterprise host and administrator access node</p>
      </div>
      ...
      >
        ...
        <div className="mb-8 text-center">
          <h3 className="text-xl font-bold text-slate-100">Sign In to Dashboard</h3>
          <p className="text-slate-400 text-xs mt-1">Access to host and manage events</p>
        </div>
              ...
              >
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="MODERATOR">Moderator (Filter chats & logs)</option>
        ...
        {/* Seed Info Box */}
        <div className="mt-8 p-4 bg-slate-900/40 rounded-xl border border-slate-800/40 text-xs text-slate-400 space-y-1">
          <p className="font-bold text-slate-300">💡 Administrator Credentials:</p>
          <p>• Admin Email: <code className="text-indigo-400">cognition@gim.ac.in</code></p>
          <p>• Password: <code className="text-indigo-400">Cognitor25!</code></p>
        </div>
      </motion.div>
    </div>
  );
